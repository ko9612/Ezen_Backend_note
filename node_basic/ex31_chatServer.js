...
io.sockets.on('connection',(socket)=>{
    ...
    socket.on('echo', (data)=>{
        ...
        //io.sockets.emit('sendAll', data);//public 통신: 접속한 모든 클에게 메시지 전송 => 클에서 sendAll이벤트를 수신(on('sendAll'))
        //socket.broadcast.emit('sendBroad',data);//broadcast통신
        io.sockets.to(uid).emit('sendOne', data);//=> private통신
    })
